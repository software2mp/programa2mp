...
replace_variables(){
	...
	REPLACEMENTS=(
		...
		"LinuxShortcut.desktop" "(^Icon=).*($)" "\/usr\/share\/applications\/${ICON}" 1					\
...
echo -e \#\!/bin/bash \\n \\nCALLNAME=\$0 \\nCALLDIR=\$\{CALLNAME\%/*\} \\nexport LD_LIBRARY_PATH=\$LD_LIBRARY_PATH:\"\$CALLDIR/../../share/lib/${EXECUTABLENAME}/lib\" \\n\"\$CALLDIR\"/"${EXECUTABLENAME}" '"$1"' > "${PACKAGEDIR}${LAUNCHSCRIPT}"
echo " [ OK ]"
...
# creo enlaces simbolico para algunas dependencias
echo -n "CREANDO ENLACES SIMBOLICOS... "
LN_NAMES=("libproj.so")
LIB_NAMES=("libproj.so.0")
CURR_DIR=$(pwd)
cd "${PACKAGEDIR}lib"
for ((ix=0; ix < ${#LN_NAMES[*]}; ++ix))
do
    ln -s "${LIB_NAMES[$ix]}" "${LN_NAMES[$ix]}"
done
cd "${CURR_DIR}"
echo "[ OK ]"
